...
- **No existe `class_sessions`**: las clases son recurrentes y fijas; las fechas concretas viven en `abril_trainer_attendance.date`. Lo puntual —suspender o mover un día— vive en `abril_trainer_class_exceptions` (0017), que solo tiene filas para lo que se sale de la norma. La ocurrencia se identifica por su fecha original; `new_date` dice adónde se movió.
- **El cobro nace de la membresía, no del alta del alumno**: asignar un plan abre el primer cobro pendiente, cobrarlo abre el del ciclo siguiente (`abril_trainer_settle_payment`) y `abril_trainer_generate_monthly_charges` es la red de seguridad idempotente. No hay cron, por el mismo motivo por el que un pago no tiene columna de estado. El ciclo se ancla al día de alta del plan.
- **`pausa` y `baja` tienen consecuencias**: no generan cobros del mes y se ven marcados al pasar lista (se los puede marcar igual: alguien en pausa puede caer un día).
